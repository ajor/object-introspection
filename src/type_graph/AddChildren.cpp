...
Pass AddChildren::createPass(DrgnParser &drgnParser, SymbolService &symbols) {
  // TODO is it ok to capture drgnParser/symbols like this?
...
void AddChildren::visit(Class &c) {
  // AddChildren should be run after Flattener
  // TODO: but this function is called recursively, so some classes will not be flattened yet
//  assert(c.parents.empty());
  ...
  for (drgn_type* drgnChild : drgnChildren) {
    ...
    auto *childClass = dynamic_cast<Class*>(childType); // TODO don't use dynamic_cast
...
//void AddChildren::enumerateClassChildren(struct drgn_type *type, std::vector<std::reference_wrapper<Class>> &children) {
